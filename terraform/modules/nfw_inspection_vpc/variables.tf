...
variable "internal_cidrs" {
  description = "CIDR blocks reachable through the transit gateway that should be treated as internal. Used for return routes from the inspection VPC back to the TGW, and merged with inspection_vpc_cidr to build the Suricata HOME_NET rule variable. Every CIDR is routed in every AZ; these are remote destinations reached via the TGW, so they have no AZ affinity. No entry may equal or be a subnet of the inspection_vpc_cidr. Entries that are supernets of the VPC CIDR, or fully disjoint from it, are both fine - ex. an aggregate such as 10.0.0.0/8 is a supernet of a 10.100.0.0/16 inspection VPC, and per-spoke CIDRs outside that range are disjoint from it. This must cover every attached spoke."
  type        = list(string)
