...
module "platform_opensearch" {
  ...
  bosh_security_group = module.stack.bosh_security_group
  listener_arn        = aws_lb_listener.main.arn
  hosts               = var.platform_opensearch_hosts
  elb_log_bucket_name = module.log_bucket.elb_bucket_name
  aws_partition       = data.aws_partition.current.partition
}
...
module "cloudwatch" {
  ...
  aws_partition                       = data.aws_partition.current.partition
  cloudwatch_lambda_function_name     = module.opensearch_cloudwatch_logs_preprocessing.cloudwatch_lambda_function_name
  metric_lambda_function_name         = module.opensearch_metrics_preprocessing.metric_lambda_function_name
}
...
module "csb_broker" {
  ...
  rds_password                    = var.csb_rds_password
  rds_subnet_group                = module.stack.rds_subnet_group
...
module "logs_opensearch_snapshot" {
  ...
  bucket          = module.logs_opensearch_snapshot_bucket.bucket
}
...
module "platform_snapshot" {
  ...
  bucket          = module.platform_opensearch.aws_s3_bucket.snapshot_bucket.bucket
}
